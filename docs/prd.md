...
| Version                 | 3.1                                                                                |
| Updated                 | 15 August 2026, 02:48 WITA                                                         |
...
### 14.0 Build-status index for FR-001–FR-082
...
12. Application, worker, proxy, provider-integration, and audit logs must never contain media bytes, object keys, signed URLs, item names, editor/free-form text, Apple credentials, OpenRouter credentials, or raw OpenRouter/upstream requests, responses, or errors containing user content.
...
- **PostgreSQL:** Accounts and anonymous links, sessions, consent versions, challenge catalog/state/completions, wardrobe items and field revisions, fingerprints, wears, import batches, media metadata/versions, jobs, tombstones, sync cursors/change tracking, deletion state, per-capability active/alternate model configuration, configuration audit history, prompt versions, pinned inference attempts, and content-free cost/outcome accounting.
...
- The MVP includes only the narrowly scoped garment-only generated illustrations in FR-070 and FR-081. It validates daily challenge-driven reuse and selected-photo backfill before investing in full-library/bulk automation, advanced editing, broader generative imagery such as outfits or people, or community.
