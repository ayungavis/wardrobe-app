            ...
            ZStack {
                ...
                if let photoData, let uiImage = UIImage(data: photoData) {
                    Image(uiImage: uiImage)
                        ...
                        .frame(width: w * photoPlacement.widthFraction, height: h * photoPlacement.heightFraction)
                        .clipShape(RoundedRectangle(cornerRadius: 4))
                        .position(x: w * photoPlacement.x, y: h * photoPlacement.y)
                }
                
                Text(completion.completedAt.formatted(date: .abbreviated, time: .omitted))
                    .font(AppFont.caption)
                    .foregroundStyle(AppColor.textSecondary)
                    .frame(width: w * completionDatePlacement.widthFraction, height: h * completionDatePlacement.heightFraction)
                    .position(x: w * completionDatePlacement.x, y: h * completionDatePlacement.y)
