...
struct DrawingPaletteTests {
    ...
    /// three widths, leaving a control that did nothing for two of its values.
    ...
    /// The swatch is drawn rather than a symbol, so its border and checkmark
    /// need a colour that reads on top of each palette entry.
    @Test func everyColourNamesAnInkThatReadsOnIt() {
        let dark: Set<DrawingColor> = [.white, .yellow, .orange]

        for color in DrawingColor.allCases {
            #expect(color.contrastInk == (dark.contains(color) ? .black : .white))
        }
    }
